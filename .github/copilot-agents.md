...
- **Test coverage improvements** Every code change MUST be covered but tests (adding unit tests, ensuring 80%+ coverage)
- **Refactoring** within architectural bounds (e.g., extract methods, improve type safety)
- **Localization updates** Every UI text change MUST be localized (adding new translation keys to ARB files, regenerating)
...
- Do not use `debugPrint()` for logging
- Use `logger` package (already configured in `lib/core/logger.dart`)
- Log levels:
  - `debug` for development-only messages
  - `info` for user-facing events
